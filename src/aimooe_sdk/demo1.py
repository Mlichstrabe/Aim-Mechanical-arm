# -*- coding: utf-8 -*-
"""
Created on Thu Jul 13 16:59:40 2021
...
try:
    from .runtime_path import aimtools_path, camera_output_dir, configure_runtime_path, require_python39
except ImportError:
    from runtime_path import aimtools_path, camera_output_dir, configure_runtime_path, require_python39
    ...
    if value == 1:
        ...
        if r == ap.E_ReturnValue.AIMOOE_OK:
            ...
            for i in range(markerNum):
                print(
                    f"{i}:({markerinfo.MarkerCoordinate[i * 3 + 0]}, {markerinfo.MarkerCoordinate[i * 3 + 1]}, {markerinfo.MarkerCoordinate[i * 3 + 2]})")
        ...
        imgDual = ap.Aim_GetMarkerStatusAndGreyImageFromHardware(aimHandle, interfaceType, pospara, markerinfo,
                                                                 hardware)
        if len(imgDual) != 0:
            ...
            leftone.save(os.path.join(camera_output_dir(), "left.bmp"))
            rightone.save(os.path.join(camera_output_dir(), "right.bmp"))
        else:
        ...
        if len(imgColor) != 0:
            ...
            colorone.save(os.path.join(camera_output_dir(), "color.bmp"))
        else:
            print("保存失败,请确认所要获取的数据已通过Aim_SetAcquireData函数正确设置并重试!")
            ...
            if r == ap.E_ReturnValue.AIMOOE_OK:
                ...
                while prlt:
                    if prlt.validflag:
                        ...
                        print("标记点坐标:")
                        print(f"工具在系统坐标系下原点坐标 (OriginCoor): {prlt.OriginCoor[0]}, {prlt.OriginCoor[1]}, {prlt.OriginCoor[2]}")
                        ...
                        print(f"工具坐标系尖端方向上另一点坐标 (toolCsmid): {prlt.toolCsmid[0]}, {prlt.toolCsmid[1]}, {prlt.toolCsmid[2]}")
                        for idx in prlt.toolptidx:
                            ...
                            if idx < 0:
                                ...
                                print(
                                    f"{markerinfo.MarkerCoordinate[idx * 3 + 0]} {markerinfo.MarkerCoordinate[idx * 3 + 1]} {markerinfo.MarkerCoordinate[idx * 3 + 2]}")
                        pnext = prlt.next
                        del prlt
                        prlt = pnext
                if len(residualIndex) > 0:
                    ...
                    for i, j in enumerate(residualIndex):
                        print(
                            f"{i}: {markerinfo.MarkerCoordinate[j * 3 + 0]}, {markerinfo.MarkerCoordinate[j * 3 + 1]}, {markerinfo.MarkerCoordinate[j * 3 + 2]}")
            else:
                del prlt
                break
        ...
        r = ap.Aim_SetToolInfoFilePath(aimHandle, toolPath)
        print("工具路径：", ap.Aim_GetToolInfoFilePath(aimHandle))
        toollist = ap.StringVector(['caliBoard', '18PT', 'cali2', 'PTM-99'])
        while True:
            r = ap.Aim_GetMarkerAndStatusFromHardware(aimHandle, interfaceType, markerinfo, hardware)
            while r == ap.E_ReturnValue.AIMOOE_NOT_REFLASH:
                time.sleep(0.01)
        ...
        if r == ap.E_ReturnValue.AIMOOE_OK:
            while True:
                ...
                if prlt.validflag:
                    ...
                    print(
                        f"相机空间的目标点在光学定位系统中的坐标：({dstPointInOpticalSys[0]}, {dstPointInOpticalSys[1]}, {dstPointInOpticalSys[2]})")
                    break
                else:
                    print("配准失败！")
        ...
        r = ap.Aim_InitToolMadeInfo(aimHandle, 18, "18PT")
        if r != ap.E_ReturnValue.AIMOOE_OK:
            print("初始化失败！")
            return
        cntTimes = 0
        ...
        r = ap.Aim_InitAccuracyCheckTool(aimHandle, "AAK-whole", "AAK-left",
                                         "AAK-right")  # "AAS-B8CD1","AAS-B4C1","AAS-B4D1")
        if r != ap.E_ReturnValue.AIMOOE_OK:
            return
        ...
        print(
            f"中心距离误差均值: {accResult[0]}  中心距离误差离散性{accResult[1]} 角度偏差均值：{accResult[2][0], accResult[2][1], accResult[2][2]}")
        ...
        for i in range(num):
            print(
                f"{i}: {data.MarkerCoordinate[i * 3 + 0]}, {data.MarkerCoordinate[i * 3 + 1]}, {data.MarkerCoordinate[i * 3 + 2]}")
            ...
            if r == ap.E_ReturnValue.AIMOOE_OK:
                ...
                while prlt:
                    if prlt.validflag:
                        ...
                        # print(f"找到工具: {prlt.toolname}  平均误差{prlt.MeanError} RMS误差{prlt.Rms}")
                        print(f"#######################第{cnt}组##########################")
                        print(f"{prlt.tooltip[0]}, {prlt.tooltip[1]}, {prlt.tooltip[2]}")
                        print(f"{prlt.toolCstip[0]}, {prlt.toolCstip[1]}, {prlt.toolCstip[2]}")
                        X = prlt.tooltip
                        Xw = prlt.toolCstip
                        Xwtmp = np.linalg.inv(prlt.Rto) @ (X - prlt.Tto)

                        print(f"{prlt.Rto}")
                        print(f"{prlt.Tto}")
                        # print(f"Tool下Xw: {(Xwtmp)[0]}, {(Xwtmp)[1]}, {(Xwtmp)[2]}")
                        # print(f"Xw误差: {(Xwtmp-Xw)[0]}, {(Xwtmp-Xw)[1]}, {(Xwtmp-Xw)[2]}")
                        time.sleep(0.5)
                    pnext = prlt.next
                    del prlt
                    prlt = pnext
...
if __name__ == "__main__":
    from src.project_paths import list_missing_aimtools

    missing = list_missing_aimtools()
    if missing:
        print("Warning: missing AimTools files:", ", ".join(missing))
        print("See AimTools/README.md — tool-related cases may fail.")
    ...
    if aimHandle is not None:
        ...
        if result == ap.E_ReturnValue.AIMOOE_OK:
            ...
            # toollist = ap.StringVector(['caliBoard', 'smallone', 'cali2'])
